...
vcf_dir = 'dna/vcf_windows/maculatus/'
dataset='dna'
...
for (sp in species){
  print(sp)
  infile = paste(vcf_dir, sp, '_100Kb_win_res.tsv', sep='')
  dat = read_tsv(infile) %>% 
    dplyr::select(-X1) %>% 
    mutate(species=sp,
           mb = BIN_START/1e6,
...
vcf_dat = purrr::reduce(sp_datlist, rbind)
vcf_dat = vcf_dat %>% 
  mutate(species = factor(species, levels=species_names)) %>% 
  dplyr::rename(chr=CHROM)

#save
out_file=paste('figure_plotting/', dataset, '_vcf_dat.Rdata', sep='')
save(vcf_dat, file=out_file)


# plot with 95 intervals --------------------------------------------------

#ratio of male snp density to female (as in Darolti)

#ratio of male snp density to female (as in Darolti)
vcf_dat %>% 
  plot_sexchrom_scatter_two_tailed(xcol='mb', ycol='mf_snpCount_ratio') +
  labs(y=bquote(log[2]~"M:F SNP density")) +
...
vcf_dat %>% 
  plot_sexchrom_scatter(xcol='mb', ycol='pFemaleSpecific', ylim=c(0,0.1)) +
  facet_wrap(~species, nrow=length(species_names))


#mean FST
vcf_dat %>% 
  plot_sexchrom_scatter(xcol='mb', ycol='MEAN_FST') +
  facet_wrap(~species, nrow=length(species_names))
