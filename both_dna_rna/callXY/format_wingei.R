...
raw_lines = read.table('both_dna_rna/pipeline_counts/raw_linecounts.txt') %>% 
  ...
  mutate(count = count/4,
         ...
         sample = sub('P.win.', 'wingei', sample, fixed=TRUE),
         sample = sub('P.win', 'wingei', sample, fixed=TRUE),
         sample = sub('Pwin', 'wingei', sample, fixed=TRUE),
         sample = sub('_WGS', '_DNA', sample),
         sample = sub('_RNA-Seq', '_RNA', sample),
         sample = sub('_H_1_', '', sample),
         sample = sub('_H_2_', '', sample),
         sample = sub('_T_', '_', sample),
         sample = sub('_1_RNA', '_RNA', sample),
         sample = sub('_2_RNA', '_RNA', sample),
         sample = sub('FDNA', '_Female_DNA', sample),
         sample = sub('MDNA', '_Male_DNA', sample),
         sample = sub('wingei_F', 'wingei_Female_', sample),
         sample = sub('wingei_M', 'wingei_Male_', sample)) %>% 
  filter(!duplicated(sample),
         grepl('wingei', sample)) %>% 
  separate(sample,
           into = c('spp', 'sex', 'num', 'source'),
           sep='_') %>% 
  mutate(sex = if_else(source=='DNA' & as.numeric(sex) > 9,
                       paste('0', sex, sep=''),
                       sex),
         sex = if_else(source=='DNA' & as.numeric(sex)< 10,
                       paste('00', sex, sep=''),
                       sex),
         spp_sex = if_else(source=='RNA',
                           paste(spp, sex, sep='_'),
                           paste(spp, num, sep='_')),
         num_source = if_else(source=='RNA',
                              paste(num, source, sep='_'),
                              paste(sex, source, sep='_')),
         sample = paste(spp_sex, num_source, sep='_')) %>% 
  select(count, sample)
