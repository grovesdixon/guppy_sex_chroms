...
raw_lines = read.table('both_dna_rna/pipeline_counts/raw_linecounts.txt') %>% 
  ...
  mutate(count = count/4,
         ...
         sample = sub('_M_', '_Male_', sample),
         sample = sub('_F_', '_Female_', sample),
         sample = sub('RNA_1_', '', sample),
         sample = sub('RNA_2_', '', sample),
         sample = sub('_1_DNA', '_DNA', sample),
         sample = sub('_2_DNA', '_DNA', sample)) %>% 
  filter(!duplicated(sample),
         grepl('reticulata', sample)) 
