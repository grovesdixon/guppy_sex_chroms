...
print(paste('m prefix =', m_prefix))
print(paste('f prefix =', f_prefix))
print(paste('out prefix =', out_prefix))
print(paste('heterogametic heterozygosity cutoff =', het_cut))
print(paste('heterogametic frequency cutoff =', heterogametic_freq_cut))
print(paste('homogametic frequency cutoff =', homogametic_freq_cut))
...
print('Reading in male frequency data...')
mfrq = read_freq(paste(m_prefix, '_afs.tsv', sep=''))
print('Reading in female frequency data...')
...
# OUTPUT SNPS AS VCF FOR SNP SPLIT ----------------------------------------

#output a VCF of the W- or Y-linked alleles
#in all cases, make the homogametic chromosome allele the Reference, and the heterogrametic chromosome allele the alternate


make_snp_vcf = function(sex_dat, strainID){
  yres = data.frame(`CHROM` = sex_dat$chr,
                    POS = sex_dat$pos,
                    ID = '.',
                    REF = if_else(sex_dat$Yallele=='alt',
                           sex_dat$ref,
                           sex_dat$alt),
                    ALT = if_else(sex_dat$Yallele=='alt',
                           sex_dat$alt,
                           sex_dat$ref),
                   QUAL = 999,
                   FILTER = 'PASS',
                   INFO = 'DP=930;DP4=283,108,361,178;CSQ=T||||intergenic_variant||||||||',
                   FORMAT = 'GT:GQ:DP:MQ0F:GP:PL:AN:MQ:DV:DP4:SP:SGB:PV4:FI',
                   strainID = '1/1:60:17:0:240,60,0:216,51,0:2:51:17:0,0,14,3:0:-0.690438:.:1') #note these are copied from mgp.v5.merged.snps_all.dbSNP142.vcf to match expectations for SNPsplit
  return(yres)
}

write_vcf_for_SNPsplit_genome_preparation = function(sex_dat, strain_tag){
  y_strainID = paste(out_prefix, strain_tag, sep='_')
  sex_vcf = make_snp_vcf(sex_dat, y_strainID)
  out_file = paste(out_prefix, paste(strain_tag, 'vcf', sep='.'), sep='_')
  print(paste('Writing out vcf file ', out_file, '...', sep=''))
  fileConn<-file(out_file)
  header_lines = paste(c('#CHROM', 'POS', 'ID', 'REF', 'ALT', 'QUAL', 'FILTER', 'INFO', 'FORMAT', y_strainID), collapse="\t")
  writeLines(header_lines, fileConn)
  close(fileConn)
  write.table(sex_vcf, out_file, sep = "\t", quote=FALSE, col.names = FALSE, row.names=FALSE, append = TRUE)
}

write_vcf_for_SNPsplit_genome_preparation(ydat, 'y')
write_vcf_for_SNPsplit_genome_preparation(wdat, 'w')

# OUTPUT SNPS FOR SNP SPLIT -----------------------------------------------

#Keeping this for reference, but found that outputting it as a VCF works better
